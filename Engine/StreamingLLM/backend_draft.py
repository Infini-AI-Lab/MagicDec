import torch
from MagicDec.Engine.StreamingLLM.model_draft import Transformer
from MagicDec.Engine.utils import load_model_draft_streamingLLM
import flashinfer
...
class LMBackend_Draft:
    ...
    def load_model(self, checkpoints: str, use_tp: bool, rank_group=None, group = None):
        self.model: Transformer = load_model_draft_streamingLLM(checkpoint_path=checkpoints, device=self.device, precision=self.dtype, use_tp=use_tp, rank_group=rank_group, group=group)        

    @torch.inference_mode()
    def setup_caches(self, max_batch_size: int = 1, draft_budget = 0):
        # Draft Budget should be 128*x+1
        self.draft_budget = draft_budget
        self.batch_size = max_batch_size
        self.cachelens = torch.zeros(max_batch_size, dtype=torch.int32, device=self.device)
        page_size = 128
        max_num_pages = (draft_budget//page_size + 1)*max_batch_size
        ...
        with torch.device(self.device):
            self.model.setup_caches(num_pages=max_num_pages, page_size=page_size, draft_budget = draft_budget)
    ...
    def encode(self, input_ids: torch.LongTensor, benchmark = False):
        ...
        for i in range(num_chunks):
            ...
            self.cachelens += dec_len
            if self.cachelens[0] >= self.draft_budget:
                self.cachelens.fill_(self.draft_budget).to(torch.int32)
            
        return logits
    
    def pre_encode(self, dec_len):
        if self.cachelens[0] + dec_len <= self.draft_budget:
            qo_indptr = self.qo_indptr*dec_len
            ...
            qo_indptr = self.qo_indptr*dec_len
            self.num_pages_per_request.fill_(self.max_num_pages_per_request).to(torch.int32)
            self.paged_kv_indices = torch.cat([torch.arange(i * self.max_num_pages_per_request, i * self.max_num_pages_per_request + self.num_pages_per_request[i], dtype=torch.int32, device=self.device) for i in range(self.batch_size)])
            self.paged_kv_indptr[1:] = torch.cumsum(self.num_pages_per_request, dim=0, dtype=torch.int32)
            self.paged_kv_last_page_len = torch.full((self.batch_size,), self.draft_budget%self.page_size, dtype=torch.int32, device=self.device)
            self.prefill_wrapper.plan(
                qo_indptr=qo_indptr,
                paged_kv_indptr=self.paged_kv_indptr,
                paged_kv_indices=self.paged_kv_indices,
